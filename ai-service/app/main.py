...
async def chat_stream(request: ChatRequest):
    ...
    async def event_generator():
        try:
            ...
            logger.error(f"Stream error for conversation {request.conversation_id}: {e}")
            error_msg = str(e)
            if "Connection" in error_msg or "ConnectError" in error_msg:
                error_msg = "Failed to connect to the LLM provider. If using Ollama on Linux, ensure OLLAMA_HOST=0.0.0.0 is set in your environment before starting Ollama."
            yield f"data: {json.dumps({'event_type': 'error', 'data': {'error': error_msg, 'conversation_id': request.conversation_id}})}\n\n"
