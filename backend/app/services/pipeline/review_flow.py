...
def is_bot_or_internal_comment(
    reviewer: str | None = None, body: str | None = None
) -> bool:
    """
    True if the comment or review was generated by a bot or by Locus itself
    (such as internal PR context analysis or inline bot suggestions).
    """
    if reviewer:
        r = reviewer.strip().lower()
        if r.endswith("[bot]") or r in ("locus-agent", "locus", "github-actions"):
            return True
        try:
            from app.services.authoring import agent_runtime
            if r == agent_runtime.agent_name().lower():
                return True
        except Exception:
            pass
    ...
    if body:
        ...
        if (
            "<!-- locus-" in b
            or "<!-- locus-pr-agent -->" in b
            or "<!-- locus-inline" in b
            or "## 🧭 locus pr context" in b
            or "## locus pr context" in b
        ):
            return True
...
async def record_review_submitted(
    ...
    """
    if is_bot_or_internal_comment(reviewer, body):
        logger.info("Ignoring bot or internal review from %r on %s#%s", reviewer, repo, pr_number)
        return None
    ...
    if not review.ticket_keys and review.pr_title:
        from app.services.pipeline import linking
        extracted = linking.extract_ticket_keys(review.pr_title)
        if not extracted and "#" in review.pr_title:
            import re
            m = re.search(r"\b([a-zA-Z0-9_.-]+/[a-zA-Z0-9_.-]+)?#(\d+)\b", review.pr_title)
            if m:
                extracted = [f"{repo}#{m.group(2)}" if not m.group(1) else f"{m.group(1)}#{m.group(2)}"]
        if extracted:
            review.ticket_keys = "\n".join(extracted)
...
def record_closed(
    ...
    Returns the review row if one was closed, or None if the pull request
